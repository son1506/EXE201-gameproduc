...
const ResetPassword: React.FC = () => {
  ...
  // Extract resetToken from URL when component mounts
  useEffect(() => {
    const tokenFromUrl = searchParams.get("resetToken"); // Get resetToken from query param
    if (tokenFromUrl) {
      setToken(tokenFromUrl);
    } else {
      message.error("Không tìm thấy mã resetToken trong URL.");
    }
  }, [searchParams]);

  const handleSubmit = async () => {
    if (!token) {
      message.error("Mã resetToken không hợp lệ.");
      return;
    }
      ...
      <div className="bg-white rounded-lg shadow-lg p-10 w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">
            Nhập mật khẩu mới của bạn để hoàn tất việc đặt lại mật khẩu.
          </p>
        </div>
